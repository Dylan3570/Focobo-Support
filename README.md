# FocusMate Support

Welcome to the official support page for the FocusMate iOS app.

## 👋 About FocusMate
FocusMate is your personal procrastination therapist and productivity coach. It helps users stay focused, manage daily tasks, and build long-term productive habits through a calming, gamified experience.

## 🛠 Support Contact
If you're experiencing issues or have questions, please contact:

**Developer:** Guanheng Wang  
**Email:** wgh357004@gmail.com  
**TestFlight Feedback:** You can also report issues directly via TestFlight.

## 📚 Common Questions

### 1. Where is my data stored?
All your data is stored **locally** on your device. No personal data is uploaded to a server.

### 2. How do I reset my tasks or points?
You can reset your data from the Settings tab in the app.

### 3. Will there be updates?
Yes! This version is part of an academic research project, and we will release improvements based on user feedback.

---

Thank you for testing FocusMate and helping improve the app!
